function save_MESA_sliding_window_to_csv(file_range, signal_you_have_chosen, window_time, window_step)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function: save_MESA_sliding_window_to_csv
...
% Example:
%   save_MESA_sliding_window_to_csv(1, 'EEG1', 10, 2);
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = file_range
    ...
    if i <= 9
        try
            [signals, header] = readEDF(sprintf('mesa-sleep-000%d.edf', i)); % For i=1 to 9
            annot_filename = sprintf('mesa-sleep-000%d-nsrr.xml', i);
    ...
    elseif i <= 99 && i>=10
        try
            [signals, header] = readEDF(sprintf('mesa-sleep-00%d.edf', i)); % For i=10 to 99
            annot_filename = sprintf('mesa-sleep-00%d-nsrr.xml', i);
            fprintf('Successfully loaded EDF and annotation files for index %d.\n', i);
        catch
            fprintf('Error loading files for index %d. Skipping this file.\n', i);
            continue;
    ...
    elseif i <= 999 && i>=100
            try
                [signals, header] = readEDF(sprintf('mesa-sleep-0%d.edf', i)); % For i=100 to 999
                annot_filename = sprintf('mesa-sleep-0%d-nsrr.xml', i);
                fprintf('Successfully loaded EDF and annotation files for index %d.\n', i);
            catch
                fprintf('Error loading files for index %d. Skipping this file.\n', i);
                continue;
            ...
            try
                [signals, header] = readEDF(sprintf('mesa-sleep-%d_ecg.edf', i));   % For i=1000 to 1289
                annot_filename = sprintf('mesa-sleep-%d-nsrr.xml', i);
                fprintf('Successfully loaded EDF and annotation files for index %d.\n', i);
            catch
                fprintf('Error loading files for index %d. Skipping this file.\n', i);
                continue;
