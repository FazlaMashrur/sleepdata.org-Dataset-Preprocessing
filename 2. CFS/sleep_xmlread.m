function [datafile, root, node] = sleep_xmlread(xmlfile, Pref)
%% default preferences
...
if isa(xmlfile, 'org.apache.xerces.dom.DeferredDocumentImpl');
  % if xmlfile is a node than skip the call to xmlread
  try
    try
      node = xmlfile;
    catch ME
...
%% parse xml file through calls to recursive node2struct function
if (Debug)   % in debuging mode crashes are allowed
  [datafile root] = node2struct(RootNode, DPref, 1);
else         % in normal mode crashes are not allowed
  try
    try
      [datafile root] = node2struct(RootNode, DPref, 1);
...
if (~isempty(GlobalTextNodes))
  GlobalTextNodes{1} = root;
  root = GlobalTextNodes;
end
%% =======================================================================
%  === node2struct Function ===========================================
%  =======================================================================
function [s TagName LeafNode] = node2struct(node, Pref, level)
...
if (node.hasChildNodes)        % children present
  ...
  for iChild = 1:nChild        % read in each child
    [c cname cLeaf] = node2struct(Child.item(iChild-1), Pref, level+1);
